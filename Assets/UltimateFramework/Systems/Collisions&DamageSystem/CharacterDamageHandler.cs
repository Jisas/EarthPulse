...
namespace UltimateFramework.CollisionsAndDamageSystem
{
    ...
    public class CharacterDamageHandler : DamageComponent
    {
        ...
        [MyBox.ConditionalField(nameof(useDamageText), false, true)]
        public TextPoints textPoints;
        [MyBox.ConditionalField(nameof(useDamageText), false, true)]
        ...
        public void ReceiveDamage(GameObject weapon, float damage, Vector3 hitDirection, Vector3 contactPoint)
        {
            ...
            if (useDamageText)
            {
                var randomNum = UnityEngine.Random.Range(0, textPoints.Value.Length);
                var currentText = Instantiate(dynamicTextPrefab, textPoints.Value[randomNum]);
                var textScript = currentText.GetComponent<CFXR_ParticleText>();
                var particles = currentText.GetComponent<ParticleSystem>();
                float intensity = damage / 1000f;
                float size = Mathf.Lerp(0.2f, 0.4f, intensity);
                Color color1 = Color.Lerp(Color.red, Color.yellow, intensity);
                textScript.UpdateText(damage.ToString(), size, color1);
                particles.Play();
            }
